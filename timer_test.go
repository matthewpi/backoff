...
func newMockTimer() backoff.Timer {
	return &mockTimer{
		started: false,
		stopped: false,
	}
...
func TestRealTimer(t *testing.T) {
	...
	// While I haven't seen this test ever return anything other than 10ms,
	// add in a margin of error just to be safe. stdlib tests should cover the
	// underlying time being correct, we just want to make sure our wrapper is
	// wired up correctly.
	if duration < 9*time.Millisecond || duration > 11*time.Millisecond {
		t.Errorf("expected timer duration to be 50ms, got \"%s\"", duration)
		return
	}
